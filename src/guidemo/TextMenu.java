...
public class TextMenu extends JMenu {
		...
		lineHeight.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				String s = JOptionPane.showInputDialog(panel,
						"What is the distance from the baseline of one line of text to the baseline of the next line below it do you want to use?",
						currentlineHeight);
				if (s != null && s.trim().length() > 0) {
					try {
						double newlineHeight = Double.parseDouble(s.trim()); // can throw NumberFormatException
						panel.getTextItem().setLineHeightMultiplier(newlineHeight); // can throw IllegalArgumentException
						panel.repaint();
					} catch (Exception e) {
						JOptionPane.showMessageDialog(panel,
								s + " is not a legal line height.\n" + "Please enter a positive integer.");
					}
				}
			}
		});
	...
	private JMenu Justify() {
		...
		String[] basic = { "CENTER", "RIGHT" };
		ButtonGroup n = new ButtonGroup();
		for (String f : basic) {
			JRadioButtonMenuItem m = new JRadioButtonMenuItem(f);
			m.setActionCommand(f);
		...
		Left.setFont(new Font("LEFT", Font.PLAIN, 12));
		n.add(Left);
		Left.setSelected(true);
		Justify.add(Left);
